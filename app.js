...
function calcCartTotals() {
  ...
  const shipping = subtotal > 0 ? 7.00 : 0; // standard $7 ($14 priority chosen at secure checkout)
  const taxRate = 0.08;
    ...
    el.innerHTML = cart.map((item, idx) => `
      <div class="cart-item">
        <div class="cart-item-info">
          ...
          ${item.options ? Object.entries(item.options).map(([k,v]) => `<div class="cart-item-detail"><span class="cart-detail-label">${k}:</span> ${v}</div>`).join('') : ''}
...
function addToCart(name, price, qty = 1, extra = null) {
  const opts = (extra && typeof extra === 'object') ? Object.assign({}, extra) : null;
  const isCustom = opts && opts.__custom; if (opts) delete opts.__custom;
  const hasOpts = opts && Object.keys(opts).length > 0;
  // Custom/optioned items are always distinct lines; plain products merge by name.
  const existing = (isCustom || hasOpts) ? null : cart.find(i => i.name === name && !i.options);
  if (existing) { existing.qty += qty; }
  else { const item = { name, price: Number(price) || 0, qty }; if (hasOpts) item.options = opts; cart.push(item); }
  renderCart();
...
document.getElementById('venmoPayBtn').addEventListener('click', function(e) {
  ...
  // Venmo profile (code) link reliably reaches Amber's account; amount entered by customer (shown in cart).
  this.href = 'https://venmo.com/code?user_id=3573264899114195665&created=1780959736';
});
...
function renderCheckoutSummary() {
  ...
  const subtotal = cart.reduce((s, i) => s + i.price * i.qty, 0);
  const shipping = subtotal > 0 ? 7.00 : 0; // standard $7 ($14 priority at checkout)
...
// ---- SECURE CARD CHECKOUT (Stripe via Netlify Functions) ----
function _slug(s){ return String(s||'item').toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-+|-+$/g,'').slice(0,60) || 'item'; }
function _itemOptions(i){
  var o = {};
  if (i.form) o['Form'] = i.form;
  if (i.size) o['Size'] = i.size;
  if (i.herbs) o['Herbs'] = i.herbs;
  if (i.symptoms) o['Symptoms'] = i.symptoms;
  if (i.options && typeof i.options === 'object') Object.assign(o, i.options);
  return o;
}
function goToSecureCheckout(){
  var items = (cart||[]).map(function(i){
    return { id: i.id || _slug(i.name), name: i.name, price: i.price, qty: i.qty, options: _itemOptions(i) };
  });
  // Include any custom items added through the soap builder (AACart) so nothing is lost.
  try {
    if (window.AACart && AACart.getItems) {
      AACart.getItems().forEach(function(a){
        items.push({ id: a.id || _slug(a.name), name: a.name, price: a.price, qty: a.qty || 1, options: a.options || {} });
      });
    }
  } catch(e){}
  ...
  var meta = {
    notes: val('checkoutNotes') || val('cartNotes'),
    name:  val('checkoutCustomerName') || val('cartName'),
    email: val('checkoutEmail') || val('cartEmail'),
    phone: val('checkoutPhone')
  };
  localStorage.setItem('aa_checkout', JSON.stringify({ cart: items, orderMeta: meta, ts: Date.now() }));
  window.location.href = '/checkout.html';
}
function startGrimoireSubscription(){ window.location.href = '/checkout.html?mode=subscription'; }
window.goToSecureCheckout = goToSecureCheckout;
window.startGrimoireSubscription = startGrimoireSubscription;

// Bind the card-checkout button (replaces the removed Shopify handler) + any subscribe buttons.
(function(){
  var b = document.getElementById('shopifyCheckoutBtn');
  if (b) b.addEventListener('click', function(e){ e.preventDefault(); goToSecureCheckout(); });
  document.querySelectorAll('[data-subscribe-grimoire]').forEach(function(el){
    el.addEventListener('click', function(e){ e.preventDefault(); startGrimoireSubscription(); });
  });
})();

async function initStripe() { return; }
