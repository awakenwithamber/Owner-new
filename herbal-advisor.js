  ...
  return '<div class="advisor-step lead-capture">' +
    ...
    '<div class="lead-form">' +
      '<label class="lead-field">' +
        '<span class="lead-field-label">Your name</span>' +
        '<input type="text" id="lead-name" autocomplete="name" placeholder="Your name" value="' + (advisorState.leadName || '') + '">' +
      '</label>' +
      '<label class="lead-field">' +
...
function submitLeadCapture() {
  ...
  var leadName = nameEl ? nameEl.value.trim() : '';

  if (!leadName) {
    if (errEl) { errEl.textContent = 'Please enter your name so Amber can personalize your report.'; errEl.style.display = 'block'; }
  ...
  advisorState.leadName = leadName;
  advisorState.leadEmail = email;
  advisorState.leadSms = sms;
  advisorState.leadSmsOptIn = optIn;
  advisorState.leadCaptured = true;

  // The full submission (name, email, phone + computed herbal allies) is sent at the
  // results step via sendQuizSubmission(), once the allies have been gathered.
...
// Sends the full quiz submission (name, email, phone + herbal allies) to the
// Netlify function, which saves to Supabase and emails the user + admins.
function sendQuizSubmission() {
  if (advisorState._quizSubmitted) return;
  if (!advisorState.leadEmail || !advisorState.leadName) return;
  var list = Array.isArray(advisorState.allies) ? advisorState.allies : [];
  var herbalAllies = list.map(function (a) {
    return a && (a.name || a.id) ? (a.name || a.id) : '';
  }).filter(Boolean);
  advisorState._quizSubmitted = true;
  try {
    fetch('/.netlify/functions/quiz-submit', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        name: advisorState.leadName,
        email: advisorState.leadEmail,
        phone: advisorState.leadSms || '',
        herbalAllies: herbalAllies,
...
function renderSynthesis() {
  ...
  try { sendQuizSubmission(); } catch (e) {}
